...
//console.log(productId)
  function handleClick() {
    navigate("/Onsale/productId="+`66d9d92357f1ac331bbdebe7`);
  }
  ...
  useEffect(() => {
    if (productId) {
      fetch(`http://localhost:4000/product/single?productid=66d9e31157f1ac331bbdec10`)
        .then((response) => response.json())
        .then((data) => {
          console.log(data);
          setProduct(data);
  ...
  return (
    ...
    cart && cart.map(( carts,index)=>
      src="/Images/1tshirt.png" 
    ( */}
      <>
        {/* <img id='shirt1121' src={`data:${cart.image.fileType};base64,${cart.image.fileContent}`} /> */}
        {/* <img id="shirt1121" src={`data:${product.image.fileType};base64,${product.image.fileContent}`} /> */}
        {/* <img id='localt' src='/Images/1tshirt.png' /> */}
        {/* <img id='localt1' src={cart.url3} />
  <img id='localt2' src={cart.url4}/> */}
        <h1 id="onelife">{product.title}</h1>
        <img id="localt3" />{product.rating}
        <div className="productprice ">
          $212
          <div className="productp">$110</div>
          <div className="productd"></div>
          <button className="b40" type="button">
            -40%
          </button>
        </div>
        <p id="productpara">{cart.description} </p>
     ...
     */}
       ...
       topsell && topsell.map((topsell,index)=>( */}
          <div className="t-shirt">
            ...
            <div className="paisa">{topsell.price}</div> */}
          </div>
        </div>
